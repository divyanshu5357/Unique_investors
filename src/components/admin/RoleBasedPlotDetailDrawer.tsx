...
export function RoleBasedPlotDetailDrawer({
    isOpen,
    onClose,
    plot,
    userRole,
    ...
    // Broker access control - only Booked and Sold plots visible
    const canBrokerAccess = ['booked', 'sold'].includes(plot.status);
    const isBrokerViewingOwnPlot = !isAdmin && canBrokerAccess;
    const isReadOnly = !isAdmin;

    // For brokers, show restrictions
    if (!isAdmin && !canBrokerAccess) {
        return (
            <Sheet open={isOpen} onOpenChange={onClose}>
                <SheetContent side="right" className="w-full sm:w-[600px] md:w-[700px]">
                    <SheetHeader>
                        ...
                        <SheetDescription>
                            You can only view Booked and Sold plots.
                        </SheetDescription>
                    </SheetHeader>
                    <div className="flex items-center justify-center h-96">
                        <div className="text-center">
                            ...
                            <p className="text-muted-foreground">
                                Plot #{plot.plotNumber} with status "{plot.status}" is not available for your view.
                            </p>
                        </div>
                    </div>
                </SheetContent>
                        ...
                        <SheetHeader className="mb-4">
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                    <SheetTitle className="text-2xl mb-2 flex items-center gap-2">
                        ...
                        {isReadOnly && (
                            <div className="bg-blue-50 border border-blue-200 rounded px-3 py-2 mb-3">
                                <p className="text-xs text-blue-800 flex items-center gap-1">
                                    ...
                                    Read-only access - View and Print only
                                </p>
                            </div>
                        )}
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <Tabs defaultValue="overview" className="w-full">
                            <TabsList className="sticky top-0 z-10 w-full justify-start border-b bg-background rounded-none px-6 h-12">
                                ...
                                {isAdmin && <TabsTrigger value="commission">Commission</TabsTrigger>}
                                {isAdmin && <TabsTrigger value="notes">Internal Notes</TabsTrigger>}
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            {plot.buyerPhone && (
                                                <div>
                                                    ...
                                                    <p className="font-medium">{plot.buyerPhone}</p>
                                                </div>
                                            )}
                                            {plot.buyerEmail && (
                                                <div>
                                                    <p className="text-xs text-muted-foreground mb-1">Email</p>
                                                    <p className="font-medium">{plot.buyerEmail}</p>
                                                </div>
                                            )}
                                        </CardContent>
                                    </Card>
                                    ...
                                    <CardContent className="space-y-3">
                                        {plot.status === 'booked' && plot.totalPlotAmount && (
                                            <>
                                                ...
                                                {plot.price && (
                                                    <div className="flex justify-between pb-2 border-b">
                                                        <span className="text-sm text-muted-foreground">Price (Per Gaj)</span>
                                                        <span className="font-semibold">₹{plot.price.toLocaleString('en-IN')}</span>
                                                    </div>
                                                )}
                                                <div className="flex justify-between pb-2 border-b">
                                                ...
                                                {plot.remainingAmount && (
                                                    <div className="flex justify-between pb-2 border-b">
                                                        <span className="text-sm text-muted-foreground">Remaining Amount</span>
                                                        <span className="font-semibold text-orange-600">₹{plot.remainingAmount.toLocaleString('en-IN')}</span>
                                                    </div>
                                                )}
                                            </>
                                        )}
                                ...
                                <TabsContent value="booking" className="space-y-4 p-6">
                                    <Card>
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            {plot.bookingDate && (
                                                <div>
                                                    ...
                                                    <p className="font-medium flex items-center gap-1">
                                                        ...
                                                        {format(new Date(plot.bookingDate), 'dd MMM yyyy')}
                                                    </p>
                                                </div>
                                            )}
                                            <div>
                                                <p className="text-xs text-muted-foreground mb-1">Booking Amount</p>
                                                <p className="font-medium text-lg text-green-600">₹{plot.bookingAmount?.toLocaleString('en-IN') || 0}</p>
                                            </div>
                                            <div>
                                            ...
                                            <Separator />
                                            <div>
                                                <p className="text-xs text-muted-foreground mb-1">Broker / Associate</p>
                                    ...
                                    <Card>
                                        ...
                                        <CardContent className="space-y-3">
                                            <div>
                                                ...
                                                <p className="font-medium flex items-center gap-1">
                                                    <CalendarDays className="h-4 w-4" />
                                                    {plot.saleDate ? format(new Date(plot.saleDate), 'dd MMM yyyy') : 'N/A'}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-muted-foreground mb-1">Sale Price</p>
                                                <p className="font-medium text-lg">₹{plot.salePrice?.toLocaleString('en-IN') || 'N/A'}</p>
                                            </div>
                                            <div>
                            ...
                            {isAdmin && plot.status === 'booked' && (
                                <TabsContent value="commission" className="space-y-4 p-6">
                                    <Card>
                                        <CardHeader>
                                            <CardTitle className="text-base flex items-center gap-2">
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-medium">{plot.commissionRate ? `${plot.commissionRate}%` : 'N/A'}</p>
                                            </div>
                                            <div>
                                ...
                                <TabsContent value="notes" className="space-y-4 p-6">
                                    <Card>
                                        <CardHeader>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-2">Admin Only - Not visible to brokers</p>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="bg-gray-50 p-4 rounded border border-dashed">
                                                <p className="text-sm text-muted-foreground italic">
                                                    No internal notes added yet. Use this space for administrative notes, issues, or follow-ups.
                                                </p>
                                            </div>
                                        </CardContent>
                                    </Card>
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="flex gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-sm font-medium">{plot.updatedBy}</p>
                                                            </div>
                                                        </div>
                                                    </>
                                                )}
